...
export default function CreateRecoveryPage() {
        ...
        if (applicationIdParam) {
          ...
          if (data.ok && data.application) {
            ...
            if (data.application.ncgc?.approved && 
              ...
              setForm(prev => ({ ...prev, applicationId: applicationIdParam }));
            } else {
              setApplications([]);
            }
          }
  ...
  useEffect(() => {
    if (form.applicationId) {
      const selectedApp = applications.find(app => app.id === form.applicationId);
        ...
        <div className="flex items-start">
          ...
          <div>
            ...
            <p className="text-sm text-blue-700">
              Recovery processes are initiated for loans where claims have been paid. 
              The goal is to recover the paid amount from the defaulting borrower through various recovery methods.
            </p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow p-6 space-y-6">
        ...
        <div>
          ...
          <select
            ...
            required
            disabled={searchParams.get("applicationId") !== null && applications.length === 1}
          >
            <option value="">Select an application...</option>
            {applications.map((app) => (
              <option key={app.id} value={app.id}>
                {app.businessName} - Claim: ₦{Number(app.claim?.payment?.amount || 0).toLocaleString()}
              </option>
            ))}
          </select>
          ...
          {searchParams.get("applicationId") && applications.length === 1 && (
            <p className="text-xs text-emerald-600 mt-1">
              Application pre-selected from claim. You can proceed to fill in recovery details.
            </p>
          )}
        </div>
        ...
        <div>
          ...
          >
            ...
            <option value="collateral_liquidation">Collateral Liquidation</option>
            <option value="legal_action">Legal Action</option>
            <option value="debt_collection_agency">Debt Collection Agency</option>
